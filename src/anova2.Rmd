...
- [Understanding interactions](#intsect)
	...
	- [Exercise 1](#ex1) including [line graphs](#linegraph)
...
To test this idea, the experimenters randomly allocated 140 people to a week-long meditation training course, while another 140 people were randomly allocated to a no-training control condition. All these people then did a word-naming task. In this worksheet, and the next one, we'll look at how to analyze data from this kind of experiment.
...
This worksheet uses the same _git_ repository as the [preprocessing](preproc.html) and [evidence, part 2](anova1.html) worksheets. Go back to your R project that contains this git repositoryh now.

Next, make sure you have loaded the data, and have loaded the R packages you need:
...
This works in much the same way as the [histogram](exploring-incomes.html#graphs), [density](group-differences.html#density), [bar](face-recog.html), and [violin](illusions.html) plots you have made before:

`onegraph %>%` - Send the filtered data to the  `ggplot` command that follows.
...
Download `interact.csv` to your local computer. Use a spreadsheet application to add in the missing reaction times for Graph 4, and upload it to RStudio. If you need to revise how to download and upload data from RStudio, see previous worksheets on [downloading](exploring-incomes.html#writing) and [uploading](entering-data-by-hand.html#upload).

Then, reload `interact.csv` using `read_csv` and use the code above to produce the graph. 

**Export your graph, download it to your local machine, and upload it to PsycEL**. If you need a reminder of how to do this, see [downloading](exploring-incomes.html#writing). 
