...
1. Open the `Files` tab. The `going-further` folder should contain the file `picture-naming-long.csv` and `music-emotion-preproc.csv`.
...
The warning `cannot compute exact p-value with ties` lets you know that the method used to calculate _p_ will not be exact, because some items in the Italian and English scores had identical rankings. It is possible to calculate the exact p value using the `pwilcox` command, but that' beyond what we'll cover in this worksheet. If your p-value is sufficiently close to .05 that it would matter if the estimate is a bit off, then a better solution would be to attempt to replicate your finding in a second study with a larger sample. 
...
The string `score by subculture` reminds you that you compared the values in `score` between the levels in the `subculture` factor. The Kruskal-Wallis _H_ statistic is `r round(kw$statistic,4)` and is derived from a chi-square distribution, using a calculation similar to the chi-squared test introduced in [the Relationships worksheet](chi.html). The degrees of freedom (`df`) are one less than the number of groups, in this case df = `r kw$parameter`. The `p` value tells us whether there was  a significant difference between the medians in any of the four groups.
