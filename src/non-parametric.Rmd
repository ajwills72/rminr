...
This worksheet covers the Mann-Whitney U test, which is an alternative to the [between-subjects t-test](evidence.html#bs-ttest), and the Kruskal-Wallis H test, which is an alternative to the [between-subjects one-factor ANOVA](anova4.html#oneBS). 
...
```
Our test scores are currently in _wide_ format (lots of columns, few rows), but R generally requires data to be in _long_ format (lots of rows, few columns). This means we first have to make the data frame wider, so we can calculate summary statistics.
