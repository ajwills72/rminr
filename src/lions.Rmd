...
motion <- animals %>% select(motion.r1, motion.r2)
```
...
**Enter the percentage agreement for your motion and location codings into PsycEL**.
...
**Enter the Cohen's kappa values for your motion and location codings into PsycEL.**
...
**On PsycEL, select the correct term to describe the kappa values for your motion and location codings.** 
...
use the `table` command we learned in the _Relationships_ worksheet to do this, but
you’re going to have to choose which behaviour, and which of your two raters,
...
If you choose "motion" and rater 2, the commands would be:

```{r cont}
cont <- table(animals$period, animals$motion.r2)
cont
```
...
In the above example graph, we can see that the animal was sleeping about as
often in the two time periods, but was lying more 10 minutes before feeding
time than 180 minutes before feeding time, and standing more 180 minutes before
feeding time than 10 minutes before. What do your data show?  Did proximity to
feeding time have an effect on behaviour?  If so, which behaviours were most
affected?
