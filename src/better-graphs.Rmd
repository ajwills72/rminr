...
dodge <- .1
w2x2 <- priming %>% ggplot(aes(x=Congruency, y=RT, group=Load, colour=Load))
w2x2 <- w2x2 + stat_summary(geom="point", fun=mean, position = position_dodge(width = dodge))
```

Line 1 defines a value that we'll use repeatedly to adjust elements horizontal position, so that they don't overlap, making the plot clearer. Line 2 defines the x axis of our plot to be the Congruency factor, the y axis to be RT, to group Congruency by Load, and to use a different colour for 'load' and 'no load'. Line 3 adds mean points to the plot using `stat_summary()`. 

`stat_summary()` is a concise way to summarise and plot graphical elements. It works in a similar way to the `group_by()` and `summarise()` pattern you've used to create summary statistics. The `fun=mean` tells `stat_summary()` to summarise the data by running the `mean` function for the grouped data you defined in the previous stop. In this case, it will calculate a mean RT (`y=RT`) for each of the groups created by `x=Congruency` and `group=Load`. `geom="point"` tells `stat_summary()` to plot the means as points. `position = position_dodge(width = dodge)` specifies an offset to use so that the points won't be plotted on top of each other. 

```{r w-pointline-2-2, message=FALSE, class.source = 'numberLines lineAnchors'}
w2x2 +
  stat_summary(geom="line", fun=mean, position = position_dodge(width = dodge)) +
...
* be clear about how CI is calculated using `stat_summary()`
