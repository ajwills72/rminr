...
In this case, error bars are useful. In the [More on t-tests](more-on-t.html) worksheet we mentioned that the 95% confidence interval is the only thing that is both useful and easy to interpret. We will plot 95% confidence intervals because, for a 2x2 within subjects design, they are calculated in such a way that they allow us to make visual comparisons between two conditions, much like a t-test. This will allow us to visually compare the main effects (congruency and load), and their [interaction](anova2.html).

```{r w-pointline-2-1, message=FALSE, class.source = 'numberLines lineAnchors'}
dodge <- .1
within_2x2 <- priming %>%
...
Line 1 defines a value that we'll use repeatedly to adjust the horizontal position of elements, so that they don't overlap. This will make the plot clearer. Line 2 defines the x axis of our plot to be the Congruency factor, the y axis to be RT, to group Congruency by Load, and to use a different colour for 'load' and 'no load'.

Line 3 adds mean points to the plot using `stat_summary()`. `stat_summary()` is a concise way to summarise and plot graphical elements. It works in a similar way to the `group_by()` and `summarise()` pattern you've used to create summary statistics. The `fun=mean` tells `stat_summary()` to summarise the data by running the `mean` function for the grouped data you defined in the previous stop. In this case, it will calculate a mean RT (`y=RT`) for each of the groups created by `x=Congruency` and `group=Load`. `geom="point"` tells `stat_summary()` to plot the means as points. `position = position_dodge(width = dodge)` specifies an offset to use so that the points won't be plotted on top of each other. Line 4 uses a similar approach to add lines to the plot, with endpoints positioned at the four mean points. We'll change the point and line colours below.

The warning `Removed 140 rows...` informs us that there are some missing values in the RT column. We'll ignore these messages, as our focus is on plotting the data.

```{r w-pointline-2-2, message=FALSE, class.source = 'numberLines lineAnchors'}
within_2x2 <- within_2x2 +
...
Line 3 adds the confidence intervals to the plot. Confidence intervals are the same length on either side of the mean. To plot the bars, `ggplot()` needs information about the two sides of the interval. One approach is to write some code to calculate the interval, then pass this data to `ggplot()` to plot the lines. However, we can do this more concisely using `stat_summary()`. The option `fun.data=` is used to specify a function that returns a `y` value, and values `ymax` and `ymin` which specify an interval above and below `y`. We can use the function `mean_cl_boot` to calculate a mean of `y`, and the associated confidence interval in `ymax` and `ymin`for our grouped data. `mean_cl_boot` calculates confidence intervals using [bootstrapping](https://en.wikipedia.org/wiki/Bootstrapping_(statistics)), which improves the interval estimate for data which isn't normally distributed.

```{r w-pointline-2-3, message=FALSE, class.source = 'numberLines lineAnchors'}
within_2x2 +
  ylab("Mean RT") + xlab("Congruency") +
  labs(caption = "Bars are 95% mean confidence intervals") +
  scale_color_grey() + theme_APA
```

### Explanation of commands

Line 2 adds meaningful labels to the axes. Line 3 adds a caption to let the reader know that the vertical bars represent 95% confidence intervals. Line 4 converts the colours to a grey pallete, and applies our APA theme.
 
### Explanation of plot

The black line is well above the grey line, showing that people were slower to respond on trials where they were rating faces. The average of the black and grey points for congruent is about 625ms, and for incongruent about 640ms. This means that RTs were similar, regardless of the congruencey between the colour and food type. The fully overlapping black bars, and partially overlapping grey bars suggest that there was no interaction between load and congruency.
...
We’ll create some plots to show the distribution of scores for the two card sets on the WinG tasks.

We start by preparing the data for plotting. These steps are described in detail in the [Better tables worksheet](better-tables.html) worksheet, so we won’t repeat them here. Instead, we’ll just list the commands and show the final output.
...
Line 1 loads the `see` package which provides the `half_violin()` function. Line 2 defines the x axis of our plot to be the WinG `task`, the y axis to be task accuracy (`correct`), and to use the `cards` factor for the fill colour. Line 3 creates a the half violin plots. `position = position_identity()` plots the two distributions on top of each other, making it easy to see how much they overlap. `alpha=0.7` changes the transparency, again to help us see the overlapping area. `size=0` removes the outline around the distributions. Line 4 gives our axes meaningful labels. Line 5 converts the fill colours to a grey pallete, and applies our APA theme. Line 6 adjusts the size of the text on the x axis, so that the task names don't overlap.
...
We'll create a boxplot to compare the emotional avoidance strategies for fans of mainstream (control group), goth, metal and emo music. This data came from a final year undergrade student dissertation.
...
FIXME: error bars for the factor of interest.
...
na %>%
    ggplot(aes(x=subculture, y=score, colour = pre_post, group = pre_post)) +
        ...
        stat_summary(geom="errorbar", width = dodge, fun.data=mean_cl_boot, position = position_dodge(width = dodge)) +
...
In addition to communicating the results of a study, graphs are also useful for visualising data patterns prior to analysis.

<a name="histogram"></a>

## Histogram and distribution curve

On a very early worksheet, we introduced the [histogram](#graphs) as a way of visualising a distribution. We can extend that plot by adding a curve to show the data distribution.

```{r histogram, class.source = 'numberLines lineAnchors', message=FALSE}
oit <- read_csv('going-further/openness-imagery-thinking.csv')

n_obs <- sum(oit$problems)
m_problems <- mean(oit$problems)
sd_problems <- sd(oit$problems)

oit %>% ggplot(aes(problems)) + 
  geom_histogram(colour = "black", binwidth = 1) +
  stat_function(fun = function(x)
      dnorm(x, mean = m_problems, sd = sd_problems) * n_obs) +
  xlim(0,7) +
...
Correlation matrices were introduced in the [Better tables](better-tables.html#cor-matrix) worksheet. We can also use a 'pairs' plot to show relationships between pairs of variables.
...
The study introduced above was interested in correlations between creative problem solving, and three other measures:

  * Openness - a common factor used in personality research
  * PsiQ - vividness of mental imagery, and
