...
* [Within participants data](#within)
    + [Point and line plot (1 factor)](#w-pointline-1)
    + [Point and line plot (2 factors)](#w-pointline-2)
* [Between participants data](#between)
    + [Half violin plot](#halfviolin)
    + [Bar plot](#bar)
    + [Boxplot](#box)
* [Mixed (between-within) data](#mixed)
...
Often involves trying out a few different graphs. Often not the first one you try. Might be something you do after inferentials.

The approach we've taken in this worksheet is to consider whether you are plotting data for within or between subjects, or a mixture of the two.

We'll demonstrate a graph for a within participants experiment using data from [an undergraduate student dissertation on the Perruchet Effect](awdiss.html).

<a name="w-pointline-1"></a>

## Point and line plot (1 factor)
...
<a name="w-pointline-2"></a>

## Point and line plot (2 factors)

* Simplifies CI syntax, but be clear what type/how CI is being calculated
...
library("ggpubr")
priming %>% ggline(x = "Congruency", y = "RT", color = "Load",
...
wing %>% ggbarplot(x = 'task', y = 'correct', color = 'cards', fill = 'cards',
 add = "mean_ci", palette = 'grey',
 position = position_dodge(), font.xtickslab = c(10),
 title = "95% mean confidence interval")
```
...
FIXME: Use a different mixed model dataset, as the interactions weren't of interest in Allegra's study. Useful for between subjects plots though.
...
na <- panas %>%
  pivot_longer(cols = c(pre_na, post_na),
               names_to = 'pre_post',
               values_to = 'score')
na %>% ggline(x = "subculture", y = "score", color = "pre_post",
                 add = "mean_ci", position = position_dodge(.1),
              linetype = 'solid',
              order = c('Mainstream', 'Emo', 'Goth', 'Metal'),
       palette = 'grey') +
...
na %>%
    ggplot(aes(x=subculture, y=score, colour = pre_post, group = pre_post)) +
      ...
      ylab("Negative affect") + xlab("Subculture") + theme_APA
```
<a name="exploring"></a>
