...
A two-factor within-subjects design is one of the most common in cognitive psychology. There is a standard way to plot this type of data, which we've previously come across in the [understanding interactions](anova2.html#intsect) worksheet.  Specifically, we use points connected by lines to represent the means in each of the four conditions. 

To do this, we first need to calculate the means for each condition, using now-familiar commands:
...
Lines 3 plots the lines on thge graph, and Line 4 adds plot points. Line 5 tells R to show the plot we have created.
