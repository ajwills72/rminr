...
The congruency effect is the difference between incongruent and congruent trials in ms. In this experiment, we're most interested in the size of the congruency effect under load. We can calculate this in a similar way to the example in the [Within-subject differences](anova1.html) worksheet.
...
Lines 1-2 pivot the data into a wide format so we can calculate the 'difference of difference'. For each participatn, lines 3-4 calculate the congruency effect under load `(Incongruent_Load - Congruent_Load)` and no load `(Incongruent_NoLoad - Congruent_NoLoad)`, then subtract the no load difference from the load difference.
...
Line 3 adds a caption to let the reader know that the vertical bars represent 95% confidence intervals. 
...
```{r w-pointline-2-2, message=FALSE, class.source = 'numberLines lineAnchors', eval=FALSE}
within_2x2 <- within_2x2 +
  stat_summary(geom="errorbar", width = dodge, fun.data=mean_cl_boot,
               position = position_dodge(width = dodge))
within_2x2
