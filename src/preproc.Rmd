...
Each CSV file (e.g. `subject-11.csv`) contains the data from one participant in a [facial prototypes](http://cabezalab.org/wp-content/uploads/2011/11/Cabeza99_PrototypeEffectFaceRecogExtensionLimits_MemCog.pdf) experiment. The experiment was run using [OpenSesame](https://osdoc.cogsci.nl/). OpenSesame, like R, is a free and open-source program. Install OpenSesame on your machine and run the experiment on yourself. Export it from the `expt-scripts` folder of your project. You export by clicking on `expt-scripts`, ticking the box next to `facialproto_short.osexp`, clicking 'More...' and clicking 'Download'. Now open it on your machine using OpenSesame.
...
Although this data file contains the participant number, this may not always be the case for other data sets. If you ever need to add participant numbers yourself, take a look at the [more on preprocessing](more-on-prepoc.html) worksheet.
...
This can, and should, be done with less than 15 files of R code. In fact, excluding the `library(tidverse)` command, it can be done in 10 lines. When your code is working correctly, it will give you the following numbers:
