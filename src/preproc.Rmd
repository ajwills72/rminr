...
Each CSV file (e.g. `subject-11.csv`) contains the data from one participant in a [facial prototypes](http://cabezalab.org/wp-content/uploads/2011/11/Cabeza99_PrototypeEffectFaceRecogExtensionLimits_MemCog.pdf) experiment. The experiment was run using [OpenSesame](https://osdoc.cogsci.nl/). OpenSesame, like R, is a free and open-source program. If you install OpenSesame on your machine, you can try out the experiment by exporting it from the `expt-scripts` folder of your project. You do this by clicking on `expt-scripts`, ticking the box next to `facialproto_short.osexp`, clicking 'More...' and clicking 'Download'.

### Summary of the experiment

You'll need a general sense of what happens in this experiment to understand the data it produces. You may find the following easier to understand if you've tried out the experiment yourself first. 

In this experiment, people are shown some pictures of male faces. Each is a picture of a real face, but its internal features (eyes, nose, mouth, etc.) have been digitally stretched or compressed either along the x-axis or the y-axis. Participants rate each picture for masculinity (1-8 scale, higher numbers = more masculine), just as a way of encouraging them to look at each picture closely.
...
Most commands in R assume your data is in _long_ format, so it's good news our raw data is also in that format, even if it still needs a bunch of tidying up before we can analyze it. The first step is to go through the 101 columns and find those we actually need.

### Responses

One thing we'll definitely need to know is how the participant responded on each trial of the experiment. 
If you scroll through the columns of `dat`, which are organized alphabetically, you'll find a column called `response`. You'll see that each row contains a number from 0 to 9. So, this is the rating the participant made on that trial. We'll need to know that to analyze this data, so `response` is one of the columns we need to keep.

Columns to keep: `response`
...
Columns to keep: `phase`, `response`

### Trial types

In the second phase of the experiment, there are three different types of trial - unseen faces, seen faces, and prototypes. We'll need to know what type of stimulus was presented in order to analyze these data. Scroll to the column called `type`, and you'll see that for the second phase (rows 33 onwards), it contains this information. For the first phase it contains `NA`, meaning this is not relevant information for the first phase. So `type` is another column we'll need to keep.

Columns to keep: `phase`, `type`, `response`

### Trial number

There are many trials in each phase of this experiment, so it makes sense to include a column that says which trial within the phase each row refers to. If you scroll through the columns, you'll find one called `live_row`. You'll notice it counts up from zero to 31, and then from 0 to 23. So, this column contains trial numbers, first for the masculinity-rating part of the experiment, and then for the confidence-rating part of the experiment. Counting from zero might seem a bit weird, but it's quite common in data collected by a computer.

Columns to keep: `phase`, `live_row`, `type`, `response`

### Column ordering

You may have noticed that the list of 'colums to keep' is not in alphabetical order. Instead, it follows the conventional ordering of _position in experiment_ (`phase, live_row`), _stimulus_ (`type`), _response_. Most experiments report their data in this order. Having conventions like these make it easier for others to read and understand our analyses. 
...
```{r select}
tidydat <- dat %>% select(phase, live_row, type, response)
```
...
Click on `tidydat` in the _Environment_ window. You'll see we have a much easier-to-read data frame, still with 56 rows, but now only 4 columns. 

### Renaming columns

When working with data, it's useful to have meaningul column names, because it makes it easier to remember what they contain. The column `live_row` could be more clearly named as `trial`, as that's the information it contains. We rename columns in R using the `colnames` command:

```{r rename}
colnames(tidydat) <- c("phase", "trial", "type", "response")
```

**Explanation of command** - The command `c()` means _concatenate_ i.e. put together. So `c("phase", "trial", "type", "response")` is a way of putting the four names together so they can be sent somewhere. They are  sent (`<-`) to the `colnames()` command, which changes the column names of `tidydat`.

## Filtering

We now have a clear, tidy dataframe (`tidydat`), which we can start to analyze. The expected result is that the participant will show high confidence ratings for having seen the prototype, despite not having seen it before. Does this participant show this pattern of results?
...
Click on `testdat` in the Envrionment window. You'll see that now we just have 24 rows, all from the test phase.

## Summarizing

We can now use the `group_by` and `summarize` commands covered in [Absolute Beginners' Guide to R](group-differences.html#group) to answer our question.

```{r summary}
testdat %>% group_by(type) %>% summarize(mean(response))
```

**Expalantion of command** - Our test-phase data (`testdat`) is piped to `group_by(type)`, which groups it into the three parts given by the `type` column (seen, unseen, prototype). This grouped data is then sent to `summarize` to work out a summary value for each group. We tell `summarize` what summary we want, in this case we want a `mean`. And we tell the `mean` command that the data we want a mean of is in the `response` column. 

Looking at the output, we can see that this participant was more confident they'd seen the prototypes (4.75) than that they'd seen the pictures they'd actually seen before (3.88). This fits our hypothesis. But, somewhat oddly, they were even more confident about other distortions they hadn't seen (5). An alternative hypothesis (and the correct one in this case) is that this particular participant just randomly pressed the buttons, so any difference in the three scores is down to chance. 

## Combining

Experiments generally involve testing relatively large numbers of people, in order to achieve good [statistical power](power.html). This means that another important part of preprocessing is to combine the data from different participants into a single data frame, so we can analyze the data from all the participants at the same time.
...
The only problem is that `alldat` does not contain any information about which data came from which participant. We often need to know this to do analyses. So, we need to add the subject number to the data frame for each subject before we combine them. We can do this using the `add_column` command, which adds columns to existing data frames.

```{r addcol}
dat <- dat %>% add_column(subj = 11, .before = TRUE)
```

**Explanation of command**: The `dat` dataframe is piped (`%>%`) to the `add_column_` command, which adds a column called `subj`, in which every row contains the number `11` (the subject number). `.before = TRUE` means it adds the `subj` column before any of the others (i.e. on the left). Without this, it would add it at the end. This data frame with an added column is written back into `dat`, so it's `dat` (rather than some new data frame) that contains the added column. 

We also need to add the subject number for  `subject-12`:

```{r addcol2}
dat2 <- dat2 %>% add_column(subj = 12, .before = TRUE)
```

Now, we combine the two data franes as before:

```{r bind2}
alldat <- bind_rows(dat, dat2)
```

Now, when you click on `alldat`, you'll see a dataframe with 112 rows and 102 columns, the first column being the subject number.

### Combining many files

Generally, we have a lot more than two people in an expeirment. You could use the same technique to combine the data of tens, hundreds, or thousands of participants. However, that would take a really long time and be quite tedious. Fortunately, R allows us to speed up this process so that, even if we have tens of thousands of participants, we can quickly combine their data without error.
...
**Line 1**: `alldat <- NULL`. This creates a new data frame called `alldat` but doesn't put anything in it yet (`NULL` means nothing, so `<- NULL` means "make it have nothing in it"). This is the data frame we'll put all the data into next.

**Line 2**: `for(fnam in fnams) {`. `fnams` is the vector of filenames we made earlier. `for(fnam in fnams) {` means "do what follows for every filename (`fnam`) in that vector of filenames. So, R will take the first filename `rawdata/subject-11.csv` and set `fnam` to that. It'll then run lines 3 and 4 (see below). When it gets to Line 5 (`}`), it will go back to line 2, set `fnam` to the second filename `rawdata/subject-12.csv`, and run lines 3 and 4 again. It'll do this over and over again until it reaches the end of the filenames in `fnamlist`

**Line 3**: `dat <- read_csv(fnam)`. This reads in one data file, just like before. The difference is that rather than writing `"rawdata/subject-11.csv"` we say `fnam`. This means that a different file will be loaded each time we go around this for loop.

**Line 4**: `alldat <- bind_rows(alldat, dat)`. This combines two data files, just like before. The difference is that `alldat` is always the first of the two files. That way, each time we go around the loop, the data file we just read gets added to the end of the files we've already combined.

**Line 5**: `}`. This means "this is the end of the loop". It's important to have this, otherwise R would try to loop around every subsequent command in your script, which would cause some weird errors. 

You may have noticed that the lines of code inside the loop (Lines 3-4) were _indented_ i.e. there are a couple of spaces before the beginning of the text. R ignores these spaces, but we include them because it makes it easier for us to see which commands are inside a loop.
...
### (Anonymously) identifying the participant

Unfortunately, `alldat` doesn't contain any information about which trials came from which participant. As before, we need to add a column that indicates which participant each row refers to. As before, we can use `add_column` to do this:

```{r combwithf, message=FALSE}
alldat <- NULL
for(fnam in fnamlist) {
  dat <- read_csv(fnam)
...
Only the fourth line of code above is new. When you run this code, you should notice that `alldat` has a new column that contains the filename. That's OK, but it would be better if we could just have the participant number (e.g. `11`) because it's more compact and easy to use like that. So, we need to be able to cut out the participant number `11` from the filename. We can do this using the `substr` command. Here's an example of how `substr` works:
...
Of course, this combined data file has the same problems as the inidividual files that make it up -- there are over 100 columns, most of which we don't need. We can fix this with the same commands as before; we just have to remember to also select the column containing the participant numbers:

```{r tidyagain}
tidydat <- alldat %>% select(subj, phase, live_row, type, response)
colnames(tidydat) <- c("subj", "phase", "trial", "type", "response")
```

Click on `tidydat`, you'll see a human-readble data fram with 168 rows, and 5 columns. This is the full dataset for the experiment, so we can now start analyzing it.

## Back to summarizing

Now we have this combined file, we summarize the test phase data for each participant using the same commands as before. We just need to include `subj` to the `group_by` command so we get a separate summary for each person:

```{r sumagain}
test <- tidydat %>% filter(phase == "test")
test.sum <- test %>% group_by(subj, type) %>% summarize(mean(response))
```

Clicking on `test.sum` in the _Environment_ window, we can now see a summary for each participant. All three participants were more confident they'd seen the prototype than the pictures they'd actually seen, which supports our hypothesis. But they're also all at least as confident about other unseen distortions, too. So, again, an alternative hypothesis is that participants were just pressing keys randomly. This was in fact the case for these data. In the next part of the worksheet, you'll look at some data from a different experiment where the participants took the task more seriously. 
...
In this experiment, words are shown on the screen one at a time. Some are real words, others are made-up words (nonwords). The participant's job is to decide whether each is a word or nonword, as quickly as possible. The computer records for each decision whether they got it correct, and how long it took them to respond (their _response time_). The experiment begins with a practice phase, so participants can get used to the task. This is followed by the test phase, which is the part we analyse.

Your task is to write R code that gives the mean reaction time for each participant, for both words and nonwords. You should analyse only reaction times for trials in which they made the correct response, and you should not include the practice phase in your analysis. 

This can, and should, be done with less than 15 files of R code. When your code is working correctly, it will give you the following numbers:
...
for(fnam in fnamlist) {
  dat <- read_csv(fnam)
  dat <- dat %>% add_column(subj = substr(fnam, 16, 17), .before = TRUE)
  alldat <- bind_rows(alldat, dat)
}
alldat <- alldat %>%
    select(subj, practice, live_row, category, correct, response_time)
colnames(alldat) <- c("subj", "practice", "trial", "type", "acc", "rt")
test <- alldat %>% filter(practice == "no") %>% filter(acc == 1) 
test.sum <- test %>% group_by(subj, type) %>% summarize(mean(rt))
test.sum
```

**Hints:** Your code should first get a list of files, then use a loop to load all those files in and combine them. Don't forget that `lexdec` is a shorter name than `rawdata`, so the `substr` command will have to change. Next you'll have to find the relevant columns among the 100+ columns of the data file. Once you've found them, use `select` to select just those columns and use `colnames` to give the selected columns better names. Use `filter` to remove the practice phase and keep only the correct responses. Note that accuracy in this data file is a number, not a word, so the correct phrase is something like `acc == 1`, rather than `acc == "1"`. Finally, use `group_by` and `summarize` to report the mean response time for words and non words for each participant. Good luck!
