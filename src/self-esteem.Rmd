...
author: "Sophie Homer, Paul Sharpe, Andy Wills"
...
The techniques used in this processing pipeline are described in more detail in the following worksheets:

* [Preprocessing data from experiments](preproc.html)
* [Preprocessing scales](preproc-scales.html)
* [Analysing scales](analyse-scales.html)
* [Within-subject differences](anova1.html)
* [Factorial differences](anova3.html)

# Introduction

This study was an experiment in which self-esteem was measured before and after participants completed one of two mental imagery conditions, or a control condition:
...
Self-esteem was measured using the State Self-Esteem Scale (SSES, Osman et al., 2012). The SSES is a 20-item survey used to measure short-lived (state) changes in self-esteem. 
...
We start by loading our data, selecting the columns we want, and tidying up the column names.

```{r preprocess-sses-pre, message=FALSE}
...
Looking at the `condition` column above, we can see that the experimental software that generated our data used numbers to represent different levels `condition` factor. It's often clearer if the numbers are converted to strings which describe the factor levels.
...
We can also look at Cronbach's alpha pre and post intervention:
...
Given the interaction, we would probably want to carry out [pairwise comparisons](anova1.html#pairs) to compare the imagery conditions against the control condition, and against each other.

Finally, to address the small difference in baseline scores, we could run the same ANOVA on the _difference_ between the pre and post scores, or we could include the baseline score as a covariate in an ANCOVA.  These techniques are outside the scope of this worksheet.
