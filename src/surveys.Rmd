...
Before totalling the SSES, we need to reverse the reverse scored items, so that a higher number means higher self-esteem for all scores. We can do this by creating a function. At this point you will be familiar with _using_ functions like `rm()` which are part of `R`, and functions like `read.csv()`, and `select()` which are provided by packages which you load using `library()` (another base `R` function). Using the `function()` function, we can define our own functions for tasks we want to do more than once. In this case we will want to reverse a number of reverse coded questions.
...
Having done this preparation, it's now easy to calculate the SSES baseline scores:
...
dass21 %>% head() %>% select(DASS1:stress)
```
...
In addition to the value of alpha, `cronbach()` returns the number of subjects and the number of items. Here were only interested in alpha, which we return in a data frame. This will allow us to use the function for calculating alpha by group.
...
We pipe this grouped data frame into `group_modify(~ sses_cronbach(.x))`. The `group_modify()` function allows you to run a function for each group in a grouped data frame. The `~` tells `group_modify()` to treat the next argument as a function, in this case `sses_cronbach(.x)`. The `.x` means the subset of rows created by our `group_by()`. The function used to process data a `group_modify()` must return a data frame. That explains why we returned alpha in a data frame in `sses_cronbach()`. So this code runs `sses_cronbach()` once for each set of rows that makes up our three conditions, with the three values of alpha combined into a single data frame.
