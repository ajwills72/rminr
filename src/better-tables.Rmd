...
Next we want to calculate an accuracy score for noun comprehension. Cells containing `C` indicate that the child could comprehend the picture on the card for noun named by this column. Cells containing `C*` indicate that the child knew a synonym for the noun. In this experiment, both of these values are considered correct responses.
...
We use similar code to create data frames containing accuracy scores for the noun production, predicate comprehension and predicate production tests. For the purposes of this worksheet, you can ignore the recoding of the `cards` column.
...
We now recombine the data for the four tests into a single data frame, in preparation for calculating our descriptive statistics:
