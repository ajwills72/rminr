...
Data is often easier to read in tabular format. This worksheet provides `R` code to produce the types of table which are frequently useful in papers describing psychological research.  Although tables can be produced using a Word processor, generating them using `R` reduces copy-paste errors when writing reports.
...
Here's a slice of the data from each sub-sheet:
...
We create a `cards` data frame and recode the `cards` factor to make it clear which card set was used with each subject. Again, we'll use `cards` later when we join our data frames together.
...
1. Any participant with the value 'N/A' for one of the first 17 words in a task. The value 'N/A' indicates that there was some kind of interruption (e.g. the child began crying). This criterion excludes children with insufficient data for analysis for a task.
1. Any participant (in the remaining data) whose accuracy was less than two standard deviations below the mean. This is a more general criterion to exlude participants who had especially low scores for various reasons.
...
1. Our function `exclude()` accepts a data frame `df` which are the raw scores for the noun and predicate comprehension and production sub-tests.
1. `logical_matrix <- df == 'N/A'` creates a matrix of values which are `TRUE` where a cell in `df` contained the string `N/A`, or `FALSE` otherwise.
1. `which(arr.ind = TRUE)` creates a matrix with columns `row` (the same as our subject number) and `col` containing the row and column number of any cells with the value `TRUE` (i.e. those cells that contained `N/A`).
1. `data.frame() %>% group_by(row) %>% summarise(min = min(col))` summarises this as a single row containing the minimum column number that contained `N/A`.
1. `mutate(subj = factor(row)) %>% select(subj, min)` creates a factor `subj` from `row`, and selects this along with the column number of the first column that contained `N/A`. We have now converted our original data into a data from with rows for any participants who had at least one `N/A` and the word number of their first `N/A`.
...
If we call `exclude(nc)`, we can see that subject `18` was excluded from the noun comprehension task:
...
We now join the data for the four word tests into a single data frame, in preparation for calculating our descriptive statistics. Notice how this sets the value `NA` for participants who were excluded for any of the tests.
...
task_by_subj <- task_by_subj %>%
  pivot_longer(cols = c(nc, np, pc, pp),
               names_to = 'task',
               values_to = 'correct')
task_by_subj %>% head(5) %>% pander(split.table = Inf)
```
...
1. Starting with our noun comprehension data `nc`, we remove the column containing the numer of `correct` answers for each participant that we generated previously.
...
We can see that all participants knew the word `apple`, regardless of which set of cards was used:
...
1. All participants had a correct score for `apple` which would produce an error on our t-test, so we remove this and similar words with `nc_long_correct %>% filter(! word %in% c('apple', 'cow', 'penguin','hat','motorbike'))`.
