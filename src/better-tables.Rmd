...
It can be helpful to presented data in tables, rather than text, especially when you need to refer to the same data in different parts of a report. Although tables can be produced manually using a Word processor, generating them directly from your data ensures they are up-to-date, and reduces copy-paste errors. This worksheet explains how to use `R` to produce the types of table used to report psychological research.
...
We'll generate a correlation matrix using the `attitude` dataset, which is included with `R`. These data are the percent proportion of favourable attitudes given by employees, in relation to seven questions regarding their department. Here are the first few rows of the data frame:
...
The first thing to notice is that the styling (spacing, use of italics, horizontal lines, positioning of captions and footnotes etc.) complies with APA guidelines for tables. You can see how the table will look when printed, using the 'print preview' feature of your word processor.

The table number and caption is above the table itself. The `Variable` column contains a number and the column name for the seven attitude variables. The next two columns show the mean and standard deviation for each variable. The remaining columns use the numbers from items in the `Variable` column as headings, indicating that they refer to the same variable. The cells show the correlation between the column variables and each of the variables in the rows. Cells are left empty where a variable would otherwise be correlated with itself. The 95% confidence interval for the correlation is shown in square brackets.

For example, the correlation between `rating` and `complaints` in this sample is .83. The confidence interval indicates that the population value could be between .66 and .91. Evidence for the correlation is calculated using traditional statistics, rather than the Bayes factors described in the [Relationships, part 2 worksheet](corr.html). One asterisk (`*`) indicates that there is less than a 1 in 20 chance that we would have found this correlation coefficient, if the correlation was actually zero (the null hypothesis). Two asterisks (`**`) signify that the probability of the null being true is less than 1 in 100.

One final thing to point out is that the correlations generated by `apa.cor.table()` are 2-tailed. One-tailed correlations are explained in the [More on relationships, part 2. worksheet](#corr-extended.html).

If you type `vignette('apaTables')` in the console, you can read about the other types of table that `apaTables` can produce.
...
Create a correlation matrix for the five personality factors. Number the table as "Table 2", and save the results in `table2.doc`. Your table should look like this:
...
This dataset has already undergone some preprocessing. We read the data, using `col_types` to explicitly define the types of data in each column. We set the first three columns as factors (the `fff`), and the remaining columns as integers (the `i`s).
...
Our test scores are currently in wide format, but we need them in long format so we can calcluate summary statistics.
