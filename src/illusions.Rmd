...
## Before you start...
Before starting this exercise, you should have completed **all** the relevant [Absolute Beginers'](index.html) workshop exercises. If not, take a look at those exercises before continuing. Each section below also indicates which of the earlier worksheets are relevant.
...
| Pattern  | Type of stimulus presented on this trial | "P1"  = Muller-Lyer, arrow heads, "P2" = Muller-Lyer arrow tails, "T" = T illusion          |
| Angle   | Angle of line | For Muller-Lyer, angle in degrees of small lines on left hand side of the Muller-Lyer stimulus, clockwise from 9 o'clock: 15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 165. For T illusion, angle in degrees of lefthand line, clockwise from 9 o'clock: 0, 15, 30, 45, 60, 75, 90, 105, 120, 135, 150, 165 |
| FinalLength  | Final length of the line you adjusted (arbitrary units) | a number; 200 if you accurately matched the other line|
| Difference200     | 200 - FinalLength                           | a number; 0 if you accurately matched, positive if your line was too short, negative if your line was too long |

You completed 240 trials in total, 120 for each illusion. Each angle was presented 10 times, except the 90 degree angle for the Muller-Lyer illusion, which was presented 20 times.
...
In this example, the participant over-esimated line length with the "arrow heads" (Angle > 90), and under-estimated line length with the "arrow tails" ( Angle < 90). Your data may be different.
...
As we've already seen (at least for our example data), the length of the line was underestimated at 45 degrees, and overestimated at 135 degrees (see the table of means, above). We also saw that the range of lengths estimated for 45 degrees barely overlapped with the range of lengths estimated for 135 degrees (see the violin plot, above). 
...
1. Produce a violin plot for the _other_ illusion type in your data file, the T illusion. How does angle affect the size of the illusion in this case?  **Save this plot as an Image, and upload it to your lab book**. 

2. Calculate the Bayes Factor for the comparison between 15 and 60 degrees for the T illusion. **Enter the exact Bayes Factor into your lab book**. 

_Hint:_ Your first step should be to produce a data frame that contains only the data from the T illusion (perhaps call it `tee`). This is the first command we covered in this worksheet. Once you've done that, the rest of the commands should work as long as you replace `mull` with `tee`. 
