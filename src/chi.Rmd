...
1. `cont <-` Store this table with the name `cont`, so we can use it later. The command `<-` stores the thing on its right in the thing of its left. 

1. `table(rows, columns)` - The R command for producing tables. We replace the word `rows` with the name of the variable we want to appear on the rows of the table, and we replace the word `columns` with the name of the variable we want to appear in the columns of the table.

2. `friends$culture` - The `culture` column of the `friends` data frame. We've put this first in our `table` command, so `culture` appears as rows.

3. `friends$coded` - The `coded` column of the `friends` data frame. This appears second in our `table` command, so `coded` appears as columns.
...
The _height_ of each tile is determined by the frequency of each of the responses (Feelings, Helping, etc.) within each culture -- the more common a response wihtin a particular culture, the taller the tile. 
...
Nonetheless, the convention is that if the _p value_ is less than 0.05, psychologists will generally believe you when your assert that the relationship is not due to chance. If the _p value_ is greater than 0.05, they will generally be skeptical. 
