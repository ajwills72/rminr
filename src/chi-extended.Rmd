...
This worksheet contains information you don't strictly need for the workshop, but which you may find useful when applying these skills later on.
...
`112.2798` - The Bayes Factor (i.e. the main result of this analysis)
...
The Bayes Factor, and chi-square, analyses only tell you that there is some kind of relationship between your two variables. They don't tell you what part of the contingency table is driving that relationship. 

For example, in the contingency table above, it looks likes the relationship is mainly being driven by the Icelandic childrens' different use of the Trust and Shared Act categories. However, the BF and X<sup>2</sup> analyses don't tell you this, they just tell you there's a relationship of some sort. 

In you want to directly test your hyopthesis, you must reduce your contingency table down to just the relevant rows and columns. So, first, you'd filter to just Shared Activities and Trust, using the `filter` command covered in the _Group Differences_ worksheet:

```{r cutdown}
ice.trust <- friends %>% filter(coded == "Shared Act" | coded == "Trust")
```

If you click on `ice.trust` in the _Enviornment_ tab of RStudio, you'll see that we now only have these two types of responses.

You also need to simply the data so it's just Iceland verus Other. We can use the `mutate` command to do this, which we haven't covered yet, but will cover in a later worksheet (_link to be inserted when worksheet is written_).

```{r cutdown2}
ice.trust <- ice.trust %>% mutate( cult2 = ifelse(culture == "Iceland", "Icleand", "Other"))
```

If you click on `ice.trust` in the _Enviornment_ tab of RStudio, you'll see that we have a new column called `cult2` that classifies countries as `Iceland` or `Other`. 

We can now re-do our analysis on this smaller contingency table:

```{r bf2, message=FALSE}
cont <- table(ice.trust$cult2, ice.trust$coded)
cont
contingencyTableBF(cont, fixedMargin = "rows", sampleType = "indepMulti")
```

The Bayes Factor for the analysis of this smaller table is well over 3, so we conclude that indeed Icelandic children differ from the other cultures we investigated on their use of the Shared Act and Trust categories. 
...
We can also calculate effect size for contingency tables. The most commonly used measure in this case Cramer's Phi, which ranges between zero and one. We use the `cramer` command in the _sjstats_ package to calculate it:
...
In this example, Cramer's Phi is about 0.34. For a contingency table that has 2 rows and 2 columns, this is conventionally described as a "medium" effect size (with 0.1 being "small" and 0.5 being "large"). For larger tables, see this [website](http://www.real-statistics.com/chi-square-and-f-distributions/effect-size-chi-square/).
...
Where there no between-group variability, Cramer's Phi is zero:
