...
### Some notes on power calcs

`vignette("pwr-vignette")` is explicit that a repeated-measures calculation of power with the `pwr.t.test` function uses mean difference divided by the standard deviation of the differences. This is sometimes known as dz (Eq 2.3.5, Cohen, 1988). 

This is not what is calculated with `cohen.d(paired=TRUE)`. The documentation is not particularly clear, and comments in source code seem wrong/contradictory, but inspection of the source code itself suggest that it just gives you standard Cohen's d, with some rounding error. Basically, it seems to calculate dz and also r, and then use the calculations on p.48-49 of Cohen (1988) to convert dz to d. Pretty weird -- why not just calculate d in the first place?

Fortunately, dz is easily calculated if you have the t statistic from a repeated-measures test (tr). It's just:
