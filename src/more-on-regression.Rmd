...
oi_lmbf1 <- lmBF(ro_i ~ age + sex + edu_sc + religious, data = data.frame(polish))
oi_lmbf1
```
...
**Note:** This Bayes Factor is for the _whole_ model (`ro_i ~ age + sex + education + religious`), not for the individual predictors that make it up (e.g. `age`). So, this analysis does not tell you that, for example, that there is evidence that `age` on its own predicts intrinsic religiosity. We'll return to this point towards the end of this worksheet.
...
The Bayes Factor for this hypothesis is over 30 million, so we can confidently claim that a model containing these spirituality variables explains more variance in intrinsic religiosity, than a simpler model with just demographics variables.
...
The Bayes Factor for this hypothesis is over 3,000, which is strong evidence that a model with the additional variables explains more variance than the simpler model in step 2.
...
```{r single}
oi_lmbf1_minus_age <- lmBF(ro_i ~ sex + edu_sc + religious, data = data.frame(polish))
