...
**NOTE:** In this section, we present the simplest possible way to perform this calculation. There
...
It is also possible to use `anovaBF` to analyse a single factor, between-subjects design. Where that factor has two levels, you could alternatively use [ttestBF](evidence.html#bayes-t). One of the useful things about `anovaBF` is that, unlike `ttestBF`, you can use it in a one-factor design that has more than two levels. For example: (**enter these commands into your script, and run them:**)
