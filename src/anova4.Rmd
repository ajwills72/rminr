...
The data for this exercise comes from a different between subjects experiment with two factors.

```{r ex1-data, message=FALSE}
cf_raw <- read_csv('going-further/counterfactual.csv')
```
...
Some of the `predictability` rows contain the value `NA`. Filter these out using `is.na()`, then calculate Bayes Factors for the main effects and the interaction. Because this data has only one observation for each participant, we can't use `subj` as a random factor, so don't include `subj` in the formula, or `whichRandom=subj`. 
...
cf_bf <- anovaBF(blame ~ perspective*predictability, data=cf)
cf_bf
cf_bf[4] / cf_bf[3]
```
