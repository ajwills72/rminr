...
Our first step will be to remove columns from the raw survey data which aren't needed for data analysis.  We'll demonstrate this using some real data from the Depression Anxiety Stress Scales—21 (DASS-21, Heatherton & Polivy, 1991). The DASS-21 is a 21-item scale for measuring depression, anxiety and stress. This data is stored in the file `going-further/dass21.csv`.
...
Some data benefits from a little more tidying than simply removing columns which aren't required. We'll demonstrate this more advanced preprocessing using the `sses.csv` dataset. This data comes from from an experiment in which self-esteem was measured before and after participants completed one of two mental imagery conditions, or a control condition. This experiment used the State Self-Esteem Scale (SSES, Osman et al., 2012), a 20-item scale used to measure short-lived (state) changes in self-esteem.
...
1. `sses_pre_raw  <- select(sses, 1, 5:25)` - We save the output of the pipeline in `sses_pre_raw`. First, we pipe the `sses` data frame into a `select` which keeps column 1, and columns 5:25. Column 1 is the participant id, columns 5:24 are the SSES scores, and column 25 is the participant's condition.
...
1. This is identical to the previous pipeline, except that the post-intervention SSES data are in columns `26:45`, these columns have the prefix `post_sse_` rather than `pre_sse_`, and we use the value `post` in our `time` factor.
