...
If you select `dass21_raw` in the **Environment** pane and look through the rest of the data, you'll see that participants 35, 49, 61, and 77 also have no data for this surve. We exclude these participants from the data frame.
...
Line 1 creates a list of the participant numbers we wish to exclude. In line 2, we remove those participants from `dass21_raw`. The `filter` command will be familiar from previous worksheets. The command `subj %in% exclude` means 'any participant whose subject number is in our list `exclude`'. The use of `!()` in the filter statement means `not`. So, `filter(!(subj %in% exclude))` means keep the participants whose subject number is _not_ in the `exclude` list. 
...
Note that we could do all of these steps in a single pipeline:
