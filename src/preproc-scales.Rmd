...
Write a similar pipeline to create a data frame named `sses_post_raw` containing the post-intervention SSES data. The post-intervention SSES data are in columns `26:45`, these columns have the prefix `post_sse_` rather than `pre_sse_`. Set the value in the `time` factor to `post`. After running your commands, the first few rows of `sses_post_raw` should look like this:
