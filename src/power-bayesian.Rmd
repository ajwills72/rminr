...
$Pr(lower bound < BF < upper bound)$. 100% is written here as one.
So if we have 40% of Bayes
...
percentages. For our purposes, the two are the same.
...
The next step is to simply give `rnorm` the mean and standard deviation of our data. We will do it for each condition
...
a 2000, inclusive. The first 1000 belong to the control condition, while the second belongs to the caffeine condition.
...
`sample` to pick 40 random numbers between 1 and a 1000 and 1001:2000
...
Now we crated two objects that contain random numbers exactly like our
participants numbers. We now filter the data by our participant numbers.
...
If the data is subset, we will need to do a Bayesian t-test on this data.
...
Let us look at the Bayes Factor.

```{r}
bf
```

This is our whole procedure for extracting a Bayes Factor for a single sample.

We see that our BF of `bf` is not conclusive. This is not promising, but we
have only sampled from our ideal data once for the sample size 40. In
principle, we could go through our procedure line by line for each sample size
many times. A more compact way to do this is to collage it together into a
_function_. Than we can use the _function_, tell it the sample size, and it will
go through the whole procedure for us.
...
This is our function. Let's try it out for a sample size of 40 with our `ideal_data`.
So we will have `n = 40` and `data = ideal_data`.
...
test we wanted. Apart from the warning message, it works flawlessly.
...
go take one sample size at a time in our tibble and record the Bayes Factor. We
would need to repeat it 20000 times to complete our power calculation.
...
and [Factorial Differences](https://www.andywills.info/rminr/anova3.html).
...
Now in order to make a graph, we will use two additional functions to show us
where is the smallest sample size that meets our criteria.

`geom_vline` will simply insert a vertical line onto the graph. A simple
vertical line can be very powerful. It will tell us exactly where to look on
the plot.
`scale_x_continuous` will let us specify how many ticks we want to draw on
the x-axis. We can use the `breaks` option within this function to let it know
what we need. We want to have more ticks than one, so that whoever is viewing
