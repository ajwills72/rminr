...
The next step is to simply give `rnorm` the mean and standard deviation of our
data. We already have these from before, so we can move on. We will do it for
each condition
...
but we have only sampled from our ideal data once only for the sample size
of 40. Randomly picking participants will result in different BFs every time we
do this.

In principle, we could go through our procedure line by line for each sample size many thousands of times. A more compact way to do this is to collage it
together into a _function_. Than we can use the _function_, tell it the sample
size, and it will go through the whole procedure for us.
