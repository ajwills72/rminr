...
For part 1 of this worksheet, see the [Absolute Beginners' Guide to R](evidence.html).
...
The data set is available in the same _git_ repository as you're already using, you just have to load it:
...
This data set is based on the design of experiments that have really been done [REF](), and is realistic in terms of its size and complexity. However, it's important to remember that this is _generated_ data, not data actually collected from participants. So, you should not take the analyses you do of this simulated data as evidence for the effectiveness of meditation in improving attentional control. As is often the case, real data in this area is less clear, and more contoversial [REF]().
...
**WRITE A COMMAND:** Use a `filter` command to extract just the control condition, and put it into a new data frame called `wordctrl`. If you need a reminder of how the `filter` command works, take a look at the [preprocessing](preproc.html#filter) worksheet.
...
**WRITE A COMMAND**: Filter out the _neutral_ trials from `wordctrl` and put the remaining data into `wordctrlCI`. You may find that the command `!=` is the most efficient way to do this. For example, if you were trying to exclude the relaxation condition, you could use `filter(medit != "relax")`. The command `!=` means 'not equal to'.
...
**WRITE A COMMAND**: Create a subject-level summary of mean reaction times, as you did [before](#summarise). However, this time, you'll need all three conditions in that summary, so use your `wordctrl` data frame, rather than your `wordctrlCI` data frame. Put your summary into a data frame called `wordctrlsum`.
...
**WRITE A COMMAND** that produces a density plot that contains all three conditions. You can do this the same way as [before](group-differences.html#group-density), just use `wordctrlsum` instead of `wordctrlsumCI`. 
...
_EXPECTED OUTPUT_: The correct output is
...
1. Start with the command `rm(list=ls()`. This command clears your _Environment_ and so ensures your script is not relying on commands you have previously run. 

2. Load the packages you need (`tidyverse` and `BayesFactor`).

3. Load the raw data (using `read_csv`).
...
First, we're using Bayesian methods, not traditional ANOVA. Although issue around multiple comparisons still occurs with Bayesian techniques, the issues are not the same, the solutions are not same, and the whole thing is just  a bit too complex for this intermediate-level worksheet.
