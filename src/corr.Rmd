...
author: "Sylvia Terbeck, Andy Wills"
...
If you want to try out this worksheet without that data file, you can use this [example CSV file](data/corr.csv) instead. _You can only gain marks in your lab book for this exercise if you use the CSV file from the workshop_.
...
We've put our answers into a new data frame, `gdata`, so go to the _Environment_ window of RStudio and click on `gdata` to see your summarised data. You'll now see one line for each group in your study.
...
"ingroup closeness and outgroup distance were significantly correlated, _r_ = .66, _BF_ = 89.7, _p_ < .05."

<a name="ex"></a>

## Exercise

In this exercise, you'll apply what you've learned to the relationship between _ingroup closeness_, and _group-leader dominance_. Do each of the following, and **upload to your lab book**:

**Hint:** Most of these steps can be completed by copying the commands you used earlier, and replacing `outgroup` with `dominance`. 

1. Make a density plot of the _dominance_ scores. Export the graph as an image, and **upload to your lab book**.

2. Make a scatterplot with _ingroup_ closeness on the x-axis, and group-leader _dominance_ on the y-axis. **Upload to your lab book**. 

3. Calculate the correlation co-efficient for _ingroup_ versus _dominance_. **Type the value into your lab book**.

4. In your lab book, **select the description of the strength of this relationship** - weak, moderate, or strong; using Cohen's conventions.

5. Calculate the Bayes Factor for this correlation. **Type the value into your lab book**. 

6. In your lab book, **select the description of this Bayes Factor** --- (a) substantial evidence for the presence of a relationship, (b) substantial evidence for the absence of the relationship, (c) equivocal evidence for the relationship. 
