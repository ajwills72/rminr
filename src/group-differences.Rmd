...
- [Calculating effect size](#effsize)
- [Exercise 1](#ex1)
- [Excerise 2](#ex2)
...
The first few steps are the same as in the _Exploring Incomes_ workshop. You open RStudio, and then you load _tidyverse_ and you load the income _data frame_. Type (or copy and paste) each of the commands in the grey box into the _Console_ window of RStudio, and press the RETURN key after each one. 
...
Looking at your output, you'll be able to see that median income for males is around $62,000, while for females it's around $53,000. (If you're curious what the rest of the output means, including the word "tibble", see [more on tibbles](more-on-tibbles.html)). 
...
In class, you were introduced to the idea of _standard deviation_, a number that basically represents how far, on average, people are from the mean. If the standard deviation of income is large, it's quite likely that a person, picked at random, will have an income that's a lot different to the mean. You can see this in the first few lines of our `cpsdata` data set -- the participant with ID 5 has an annual income of over $700k; more than 12 times the median income!
...
We do this using the `filter` command. It's called _filter_ because it works a bit like the filter paper in a chemsitry lab (or in your coffee machine) -- stopping some things, while letting other things pass through. We can filter our data by telling R *what data we want to keep*. Here, we want to keep all people who earn less than £150k, and filter out the rest. So the filter we need is `filter(income < 150000)`, where `<` means "less than". 
...
**So start with the command** `cpslow %>% ggplot(aes(income, colour=factor(sex))) + geom_density(aes(y=..scaled..))`, **make that change, and press RETURN.** Remember that you can get back to commands you typed earlier by pressing the up arrow on your keyboard while in the Console. 
...
As we covered in class, *effect size* is a way of talking about the size of the difference between group means, relative to the standard deviations of those groups. We often use the letter *d* to stand for effect size. 
