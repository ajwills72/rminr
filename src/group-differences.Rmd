...
The first few steps are the same as in the _Exploring Incomes_ workshop. First, log in to <a href = "https://rstudio.plymouth.ac.uk" target = "blank">RStudio server</a>, and make sure you are in your `psyc411` project.

Next, create a **new** script file in your project, called `groupdiff.R`.

Now, you load _tidyverse_ and you load the income _data frame_, by adding the following commands to your R script, and using CTRL+ENTER to run each command in turn:
...
So start with the command `cpsdata %>% ggplot(aes(income, colour=factor(sex))) + geom_density(aes(y=..scaled..))`, **make that change, add it to your script, and run it.**.
...
**So, add this command to your script and run it**:
...
**Add commands to do this to the end of your script to do this, and run them**. Below are the answers you are aiming for:
...
**Add commands to complete this exercise to the end of your script, and run them**. Below are the answers you are aiming for:
