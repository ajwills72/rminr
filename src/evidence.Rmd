...
Before collecting any data, this hypotheses seems unlikely. If the experiment is well designed (see lectures), we might conclude that the probability of the null hypothesis was rather high. Let's say, P(null) = 0.95. 
...
The easiest way to do a Bayesian t-test in R is to use the _BayesFactor_ package. Recall, that you load a package using the `library` command. The `quietly = TRUE` bit is there because this package is very chatty when it loads, which gets a bit annoying after a while.
