...
- [Plotting WinG task scores](#plot-task)
...
ggplot(task_by_subj_l, aes(x = task, y = correct, fill = cards)) +
  ...
  scale_fill_material_d() + xlab('WinG Task') + ylab('Score (max = 20)')
```
...
We plotted the data in this format to get an indication of whether there were differences between the Italian and English cards on any of the tests. This looks unliely, given the extensive overlap in scores between the card sets. The plot also shows that the data were slightly skewed on the noun tasks due to some low scores.
...
Finally, we would like to know if there were any differences for the semantically related errors between the two sets of cards.
