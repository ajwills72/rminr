...
We'll create a plot to show the distribution of scores for the two card sets on the WinG tasks.
...
Finally, we would like to know if there were any accuracy differences between individual Italian and English cards in each task. We can summarise this data as a table of t-tests.
...
t_words <- function(df, group) {
  result <- df %>% group_by(cards) %>% summarise(mean = mean(correct), sd = sd(correct))
  result <- pivot_wider(result, names_from = cards, values_from = c(mean,sd))
  bf <- ttestBF(formula=correct ~ cards, data = data.frame(df))
  t <- t.test(correct ~ cards, data = df)
  p <- t$p.value
  result <- result %>% mutate(bf = extractBF(bf)$bf, t = t$statistic, p = p)
  return(result)
}
```
...
The `correct_word` function returns a data frame with a column `correct` which is the number of correct responses in the column named `answer`. This is like the `rowSums()` command we used to calculate a correct score for each participant. We use `colSums()` because the ratings are in the `answer` column.

The `t_words` function is like the `t_gender` function we created earlier. We'll use this function to return a Bayes factor comparing the accuracy for each card between the Italian and English pictures. 
...
nc_bf <- nc_long_correct %>%
  ...
  mutate_if(is.numeric, round, 2) %>%
  select(word, mean_english, sd_english, mean_italian, sd_italian, t, p, bf) %>%
  arrange(bf)

nc_bf %>% kable(
  col.names = c('Word', 'Mean (English)', 'SD (English)', 'Mean (Italian)', 'SD (Italian)', 't', 'p', 'BF')) %>%
  kable_styling()
```

**Explanation of commands:**

Running a t-test where both groups have identical scores produces an error, so we remove these cards from our tests. For noun comprehension, this was the case for the words 'apple', 'cow', 'penguin','hat', and 'motorbike'.
