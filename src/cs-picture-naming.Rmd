...
We define the function `t_gender()` to run a between-subjects t-test comparing task accuracy (`correct`) by `gender`. Before running the test, we use `drop_na(df)` to remove rows for participants who were excluded for the task. The function returns the Bayes factor. We group our long format task data by task, and use `group_modify(t_gender)` to calculate the Bayes factor for the grouped data. `group_modify()` passes grouped data to a function, and replaces it with the data frame returned by the function. We round the Bayes factors to 2 decimal places and then select the `task` and `bf` columns. We join this to our `descriptives` data frame.
...
task_by_subj_l$task <- recode_factor(task_by_subj_l$task,
...
The 'non-taget but semantically related' code only relates to the production tasks, so we select just the data for these tests in line 2. Lines 3-5 convert the semantically related error scores from wide to long format. Notice that we use `values_to` to more these scores to a column named `correct`. This will allow us to reuse the `mann_whitney` function again. Lines 6-7 give the factor levels meaningful names. Lines 9-12 generate half violin plots for the production tasks.
...
In summary, these data suggest that WinG scores should be comparable, regardless of whether the English or Italian picture cards are used. It also shows that adult's ratings of how the pictures map to the underlying concepts are quite different to children's.
