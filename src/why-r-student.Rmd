...
options(scipen=999)
citline  <- cit %>%
    ggplot(aes(x=year, y=scholar2023 * 1000, group=software, colour=software)) +
...
### Never out of date
Every analysis you can think of is already available in R, thanks to over [19,000 free packages](https://cran.rstudio.com/). As new analyses are developed, they become available in R first. In 2013, SPSS realised it couldn't keep up with R, and [admitted defeat](https://www.ibm.com/developerworks/library/ba-call-r-spss/index.html).
...
You can use R without having to install it, e.g. [RStudio Plymouth](https://psyrstudio.plymouth.ac.uk). 
...
systems are substantively limited when it comes to data preprocessing, and data visualization.
They also do not encourage a
...
companies including [posit](https://posit.co/) (who provide the RStudio interface).
