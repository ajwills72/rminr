...
- [Excluding participants on a criterion](#exclude)
...
Our data is in long format i.e. one observation per row. The four columns are as follows:
...
- `key` : The type of observation being made. Mostly, these are `GS` observations, which are _galvanic skin responses_. These are measures of level of arousal in the participant. There are also `ES` observations, which are the participant's ratings

- `value` : The actual observation. Where `key` is `GS`, these are galvanic skin response values, which are measures of conductance (the reciprocal of electrical resistance). Higher values indicate higher arousal. 
...
Line 2, `raw <- distinct(raw)` is the _de-duplication_ command. 
...
## Excluding participants on a criterion
