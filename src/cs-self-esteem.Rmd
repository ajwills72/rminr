...
1. We select only the pre-intervention scores with `filter(time == 'pre')` because, assuming the interventions were successful, the post-intervention scores would reduce alpha.
...
bf <- anovaBF(formula = total ~ condition, data = data.frame(sses_pre))
bf
```
...
We use `extractBF()` to extract Bayes factors from the results of `anovaBF`. The two main effects come from `bf[1]` and `bf[2]`. The interaction is the results of `bf[4] / bf[3]`, which can also be passed to `extractBF()`. We `round()` each of the Bayes factors to 2 decimal places.

### Explanation of output

The Bayes factor for `time` (_BF_ = `r time_bf`) tells us that there was no overall change in self-esteem after the intervention relative to baseline. The Bayes factor for `condition` (_BF_ = `r condition_bf`) tells us that there were overall differences in self-esteem between the three conditions. The `interaction_bf` of `r interaction_bf` tells us that it's almost 10 times more like that there's an interaction between `condition` and `time` than that there isn't.
